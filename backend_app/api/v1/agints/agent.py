...
# Import schemas, core logic, and dependencies
from ...schemas import Agent, AgentCreate, SignedRequest, UserInDB
...
def read_agent(agent_id: int, db: Session = Depends(get_db), current_user: UserInDB = Depends(get_current_user)):
    """Retrieve a specific agent owned by the current user."""
...
def read_all_agents(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: UserInDB = Depends(get_current_user)):
    """Retrieve a list of all agents owned by the current user."""
