...
def read_all_agents(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: UserInDB = Depends(get_current_user)):
...
def update_existing_agent(
    agent_id: int, 
    request: SignedRequest[AgentUpdate], 
    db: Session = Depends(get_db), 
    current_user: UserInDB = Depends(get_current_user)
):
    """Update an existing agent's details, verified by the user's signature."""
    db_agent = agent_manager.get_agent(db, agent_id=agent_id)
    if not db_agent or db_agent.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Agent not found")
    ...
    agent_update_data = request.payload.data
    return agent_manager.update_agent(db=db, agent_id=agent_id, agent_update=agent_update_data)

@router.delete("/{agent_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_existing_agent(
    agent_id: int, 
    request: SignedRequest[Any], 
    db: Session = Depends(get_db), 
    current_user: UserInDB = Depends(get_current_user)
):
    """Delete an agent, verified by the user's signature."""
    db_agent = agent_manager.get_agent(db, agent_id=agent_id)
    if not db_agent or db_agent.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Agent not found")
    ...
    agent_manager.delete_agent(db=db, agent_id=agent_id)
